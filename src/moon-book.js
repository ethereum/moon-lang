...
var req = (value, cache) => () => (cache || (cache = require(value))); // temporary hack, TODO: use "browser" on package.json
var fs = req("fs");
var path = req("path");
...
var resolve = (termName, opts = {}) => {
  return new Promise((resolve,reject) => {
    ...
    var termKey = isSwarmHash
      ? termName
      : asciiToBytes32(termName);

    // If there is a chache (browser's localStore or disk), return it
    if (!opts.disableCache) {
      var termId = isSwarmHash
        ? "moon-base-swarm-" + termName
        : "moon-base-testnet-" + termName;
      if (onBrowser) {
    ...
    if (!isSwarmHash) {
      ...
      swarmHashRequest.then(swarmHashResult => {
    ...
    // If termName is a swarmHash, get its source
      ...
      return termSource.then(termSource => {
        ...
        var errors = [
          "error resolving "+termName+": no contract code at given address\n",
          "404 page not found\n"
        ];
