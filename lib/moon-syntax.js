...
var termFormatter = function termFormatter(decorations) {
  return function (term) {
    ...
    var formattable = term({
      App: function App(f, x) {
        // (App [App f [x]] y) -> (App f [x y])
        if (f.term[0] === _App) {
          return {
            term: [_App, f.term[1], f.term[2].concat([x])],
      ...
      Lam: function Lam(name, body) {
        ...
        if (body.term[0] === _Lam) {
          return {
            term: [_Lam, [name].concat(body.term[1]), body.term[2]],
      ...
      Let: function Let(name, term, body) {
        ...
        if (body.term[0] === _Let) {
          return {
            term: [_Let, [[name, term]].concat(body.term[1]), body.term[2]],
    ...
    var stringifyFlat = function stringifyFlat(_ref15) {
      var term = _ref15.term,
          size = _ref15.size;

      var go = function go(_ref16) {
        var term = _ref16.term,
            size = _ref16.size;
        ...
        switch (term[0]) {
          ...
          case _Ref:
            return D.Ref(term[1]);
          case _Let:
            return D.Many([D.Many(term[1].map(function (_ref17) {
          ...
          case _Map:
            return D.Many([D.Text("{"), D.Many(term[1].map(function (_ref19, i) {
    ...
    var stringify = function stringify(_ref21) {
      var term = _ref21.term,
          size = _ref21.size;
      ...
      var go = function go(_ref22) {
        var term = _ref22.term,
            size = _ref22.size;
          ...
          switch (term[0]) {
            ...
            case _Let:
              for (var i = 0; i < term[1].length; ++i) {
                prepend(D.Many([D.Var(term[1][i][0]), " = "]), term[1][i][0].length + 2);
                go(term[1][i][1]);
              }
              go(term[2]);
              break;
            ...
            case _Map:
              ...
              tab();
              for (var i = 0; i < term[1].length; ++i) {
                prepend(D.Many([D.Key(term[1][i][0]), D.Text(":"), D.Text(" ")]), term[1][i][0].length + 2);
                go(term[1][i][1]);
              }
              untab();
              push(D.Text("}"));
    ...
    return D.indent ? stringify(formattable) : stringifyFlat(formattable);
  };
};

var termToString = function termToString(term, spaces, maxCols) {
